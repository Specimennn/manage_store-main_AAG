...
public class SaleService {
    ...
    public List<Store> getStoresAboveSaleAmount(int amount){
        List <Store> storesAbove = new ArrayList<>();
        for (Sale sale:sales) {
            if (getTotalSalesByStore(sale.getStore().getCode())> amount) {
                storesAbove.add(sale.getStore());
            }
        }
